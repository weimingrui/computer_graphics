...
GLfloat high_shininess[] = {100.0};GLfloat high_shininess1[] = {1.0,100.0};
...
glEnable(GL_DEPTH_TEST); 
 ...
 static GLfloat position1[] = { 0.0, 0.0, 5.0}; 
...
{   
 ...
 glPushMatrix (); 
gluLookAt(position1[0],position1[1],position1[2],centx,centy,centz,0.0,1.0,0.0);
...
void keys(unsigned char key,int x,int y)
{
	...
	if(key=='Q')position1[0]-=1.0;
	if(key=='W')position1[0]+=1.0;
	if(key=='E')position1[1]-=1.0;
	if(key=='A')position1[1]+=1.0;
	if(key=='S')position1[2]-=1.0;
	if(key=='D')position1[2]+=1.0;

	if(key=='C')mat_specular[0] -=1.0;
    if(key=='Z')ambient[0]+=0.1;
	 if(key=='B')mat_diffuse[0] += 1.0;
    if(key=='X')high_shininess[0] -=100.0 ;
	//if(key=='V')glMaterialfv(GL_FRONT,GL_SHININESS,high_shininess1);//glMaterialfv(GL_FRONT, GL_AMBIENT,mat_red_ambient);
	display();
}
